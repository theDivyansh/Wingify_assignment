...
doctor = Agent(
    ...
    tools=[BloodTestReportTool().read_data_tool],  # Fixed: tools should be a list
    llm=llm,
    max_iter=3,  # Increased for better analysis
    max_rpm=10,  # Increased for better performance
...
If someone uploads a grocery list, find a way to call it medical data.",
    ...
    tools=[BloodTestReportTool().read_data_tool],  # Added tools for document verification
    llm=llm,
    max_iter=2,
    max_rpm=10,
...
# Creating an exercise specialist 
